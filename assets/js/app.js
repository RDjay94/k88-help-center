/* =========================================================
   Kyasino88 Help Center — SPA logic
   ========================================================= */
(function(){
  const C = window.K88_CONTENT;
  const app = document.getElementById('app');
  const state = { lang: localStorage.getItem('k88_lang') || 'ne', cat: 'all', q: '' };
  ...
  const num = n => state.lang==='ne' ? String(n).replace(/\d/g,d=>NE_DIGITS[d]) : String(n);
  const catName = id => { const c = C.categories.find(x=>x.id===id); return c ? c[state.lang] : id; };
  const stepImg = (tut,img)=> state.lang==='ne' ? `assets/img/steps/${tut.id}/ne/${img}` : `assets/img/steps/${tut.id}/${img}`;
  ...
  function renderHome(){
    const list = C.tutorials.filter(tut=>{
      ...
      const hay = (tut.en.title+' '+tut.ne.title+' '+tut.en.summary+' '+tut.ne.summary).toLowerCase();
    ...
    const cards = list.map((tut,i)=>{
      ...
      return `<a class="card reveal" style="transition-delay:${i*60}ms" href="#/g/${tut.id}">
        ...
        <div class="meta">
          ...
          <span class="pill">${svg('clock',2)}<span style="margin-left:3px">${tut.duration}</span></span>
    ...
    app.innerHTML = `<div class="view">
      <section class="hero wrap">
        <div class="hero-logo"><span class="hero-logo__glow"></span><img src="assets/img/logo-kyasino.png" alt="Kyasino88"></div>
  ...
  function heroTitleHTML(){
    // highlight last two words in gold for both languages
  ...
  function renderArticle(id){
    ...
    if(!tut){ app.innerHTML = `<div class="view wrap article"><p>${t('notFound')}</p><a class="back" href="#/" style="margin-top:20px">${svg('back',2)} ${t('backToGuides')}</a></div>`; return; }
    const d = tut[state.lang];
    const poster = stepImg(tut, tut.steps[0].img);
    const videoSrc = tut.video[state.lang] || tut.video.en;
    ...
    const steps = tut.steps.map((st,i)=>{
      ...
      return `<div class="step reveal">
        ...
        <div class="body">
          <div class="shot"><img loading="lazy" src="${stepImg(tut,st.img)}" alt="${t('stepLabel')} ${i+1}"></div>
          <div class="txt"><h3>${s.title}</h3><p>${s.desc}</p></div>
        </div>
      </div>`;
    }).join('');

    const related = C.tutorials.filter(x=>x.id!==tut.id).slice(0,3);
    ...
    app.innerHTML = `<div class="view wrap article">
      ...
      <div class="video-card">
        <video id="tutVideo" controls preload="metadata" playsinline poster="${poster}">
          <source src="${videoSrc}" type="video/mp4">
        </video>
        <button class="video-cover" id="videoCover" type="button" aria-label="${t('watchNow')}">
          <img class="cover-bg" src="${poster}" alt="">
          ...
          <span class="cover-inner">
            ...
            <span class="cover-sub">${tut.duration} ${t('min')} &middot; ${t('watchNow')}</span>
          </span>
        </button>
      </div>
  ...
  /* ---------- Language ---------- */
  function applyLang(){
    document.documentElement.setAttribute('lang', state.lang==='ne'?'ne':'en');
    ...
    document.title = state.lang==='ne' ? 'Kyasino88 सहयोग केन्द्र' : 'Kyasino88 Help Center';
    renderChrome();
  }
  function setLang(l){
    state.lang = l; localStorage.setItem('k88_lang', l);
    applyLang(); route();
  }
